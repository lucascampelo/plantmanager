...
export async function savePlant(plant: PlantProps) {
    try {
        ...
        const notificationId = await Notificacions.scheduleNotificationAsync({
            content: {
                title: 'Heeey, 🌱',
                body: `Está na hora de cuidar da sua ${plant.name}`,
                sound: true,
                priority: Notificacions.AndroidNotificationPriority.HIGH,
                data: {
                    plant
                }
            },
...
export async function removePlant(id: string): Promise<void> {
    ...
    await Notificacions.cancelScheduledNotificationAsync(plants[id].notificationId);
    delete plants[id];
